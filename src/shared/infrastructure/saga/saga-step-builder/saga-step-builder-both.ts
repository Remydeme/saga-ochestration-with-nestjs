...
export type SagaDefinition = {
    channelName: string,
    phases: { [key in StepPhase]?: { action: Action<any, string> } }
}
...
export class SagaDefinitionBuilder {
    ...
    onReply(action:  Action<any, string>): SagaDefinitionBuilder {
        this.checkIndex();
        this.sagaDefinitions[this.index!].phases[StepPhase.StepForward] = {action: action};
        return this;
    }

    withCompensation(action:  Action<any, string>): SagaDefinitionBuilder {
        this.checkIndex();
